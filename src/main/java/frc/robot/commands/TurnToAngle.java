...
public class TurnToAngle extends CommandBase {
  ...
  {
    speedTuner = new PIDController(SmartDashboard.getNumber("TURN KP", turn_kP), 
                                  SmartDashboard.getNumber("TURN KI", turn_kI), 
                                  turn_kD);
                                  
    // speedTuner = new PIDController(0.5, 
    // 0.05, 
    // turn_kD);
  }
  ...
  public void execute() 
  {
    current_error = tAngle - __drivebase.getGyroYaw();
    double output = speedTuner.output(current_error);
    double current_speed =  MathUtil.clamp(output, -0.5, 0.5);
